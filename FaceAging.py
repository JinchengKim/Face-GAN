...
class FaceAging(object):
    ...
    def single_test(self, images, gender, name, age):
        test_dir = os.path.join(self.save_dir, 'test')
        if not os.path.exists(test_dir):
            os.makedirs(test_dir)
        images = images[:int(np.sqrt(self.size_batch)), :, :, :]
        ...
        save_images_single(
            batch_images=query_images,
            save_path=os.path.join(test_dir, 'single_input.png'),
            image_value_range=self.image_value_range,
            size_frame=[size_sample, size_sample],
            index = age
        )
    ...
    def custom_test_single_image(self, image_path, label = 0):
        ...
        self.single_test(images, gender_male, 'test_as_male.png', label)
        self.single_test(images, gender_female, 'test_as_female.png', label)
        pass
